...
func (s *saldoStatsTotalBalanceService) FindMonthlyTotalSaldoBalance(ctx context.Context, req *requests.MonthTotalSaldoBalance) ([]*models.MonthlyTotalSaldoBalanceRow, error) {
	const method = "FindMonthlyTotalSaldoBalance"
	ctx, span, end, status, logSuccess := s.observability.StartTracingAndLogging(ctx, method, attribute.Int("year", req.Year), attribute.Int("month", req.Month))
	...
	if err != nil {
		status = "error"
		return errorhandler.HandleError[[]*models.MonthlyTotalSaldoBalanceRow](s.logger, saldo_errors.ErrFailedFindMonthlyTotalSaldoBalance, method, span, zap.Int("year", req.Year), zap.Int("month", req.Month))
	}
...
func (s *saldoStatsTotalBalanceService) FindYearTotalSaldoBalance(ctx context.Context, year int) ([]*models.YearlyTotalSaldoBalancesRow, error) {
	...
	if err != nil {
		status = "error"
		return errorhandler.HandleError[[]*models.YearlyTotalSaldoBalancesRow](s.logger, saldo_errors.ErrFailedFindYearTotalSaldoBalance, method, span, zap.Int("year", year))
	}
