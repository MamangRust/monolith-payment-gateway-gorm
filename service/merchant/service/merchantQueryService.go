...
func (s *merchantQueryService) FindAll(ctx context.Context, req *requests.FindAllMerchants) ([]*models.MerchantListRow, *int, error) {
	...
	if err != nil {
		status = "error"
		return errorhandler.HandlerErrorPagination[[]*models.MerchantListRow](s.logger, err, method, span, zap.String("search", search))
	}
...
func (s *merchantQueryService) FindById(ctx context.Context, merchantID int) (*models.MerchantAllFieldsRow, error) {
	...
	if err != nil {
		status = "error"
		return errorhandler.HandleError[*models.MerchantAllFieldsRow](s.logger, err, method, span, zap.Int("merchant.id", merchantID))
	}
...
func (s *merchantQueryService) FindByActive(ctx context.Context, req *requests.FindAllMerchants) ([]*models.MerchantListWithDeletedRow, *int, error) {
	...
	if err != nil {
		status = "error"
		return errorhandler.HandlerErrorPagination[[]*models.MerchantListWithDeletedRow](s.logger, err, method, span, zap.String("search", search))
	}
...
func (s *merchantQueryService) FindByTrashed(ctx context.Context, req *requests.FindAllMerchants) ([]*models.MerchantListWithDeletedRow, *int, error) {
...
func (s *merchantQueryService) FindByApiKey(ctx context.Context, apiKey string) (*models.MerchantAllFieldsRow, error) {
	const method = "FindByApiKey"

	ctx, span, end, status, logSuccess := s.observability.StartTracingAndLogging(ctx, method, attribute.String("api_key", observability.MaskIdentifier(apiKey)))
	defer func() { end(status) }()

	if cachedMerchant, found := s.cache.GetCachedMerchantByApiKey(ctx, apiKey); found {
		logSuccess("Successfully found merchant by API key from cache", zap.String("api_key", observability.MaskIdentifier(apiKey)))
		return cachedMerchant, nil
	}
	...
	if err != nil {
		status = "error"
		return errorhandler.HandleError[*models.MerchantAllFieldsRow](s.logger, err, method, span, zap.String("api_key", observability.MaskIdentifier(apiKey)))
	}

	s.cache.SetCachedMerchantByApiKey(ctx, apiKey, merchant)

	logSuccess("Successfully found merchant by API key", zap.String("api_key", observability.MaskIdentifier(apiKey)))

	return merchant, nil
}

func (s *merchantQueryService) FindByMerchantUserId(ctx context.Context, userID int) ([]*models.MerchantListByUserRow, error) {
	...
	if err != nil {
		status = "error"
		return errorhandler.HandleError[[]*models.MerchantListByUserRow](s.logger, err, method, span, zap.Int("user.id", userID))
	}
