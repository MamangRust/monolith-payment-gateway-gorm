...
func (s *topupStatsByCardMethodService) FindMonthlyTopupMethodsByCardNumber(ctx context.Context, req *requests.YearMonthMethod) ([]*models.TopupMonthlyMethodByCardRow, error) {
	...
	if data, found := s.cache.GetMonthlyTopupMethodsByCardNumberCache(ctx, req); found {
		logSuccess("Successfully fetched monthly topup methods by card number", zap.String("card_number", observability.MaskIdentifier(req.CardNumber)), zap.Int("year", req.Year))
		return data, nil
	}
	...
	if err != nil {
		status = "error"
		return errorhandler.HandleError[[]*models.TopupMonthlyMethodByCardRow](
			s.logger,
...
func (s *topupStatsByCardMethodService) FindYearlyTopupMethodsByCardNumber(ctx context.Context, req *requests.YearMonthMethod) ([]*models.TopupYearlyMethodByCardRow, error) {
	...
	if data, found := s.cache.GetYearlyTopupMethodsByCardNumberCache(ctx, req); found {
		logSuccess("Successfully fetched yearly topup methods by card number", zap.String("card_number", observability.MaskIdentifier(req.CardNumber)), zap.Int("year", req.Year))
		return data, nil
	}
	...
	if err != nil {
		status = "error"
		return errorhandler.HandleError[[]*models.TopupYearlyMethodByCardRow](
			s.logger,
