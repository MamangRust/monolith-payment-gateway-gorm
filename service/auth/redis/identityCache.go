...
func (c *identityCache) SetCachedUserInfo(ctx context.Context, user *models.UserByIDRow, expiration time.Duration) {
	if user == nil {
		return
	}
...
func (c *identityCache) GetCachedUserInfo(ctx context.Context, userId string) (*models.UserByIDRow, bool) {
	key := fmt.Sprintf(keyIdentityUserInfo, userId)

	return sharedcachehelpers.GetFromCache[models.UserByIDRow](ctx, c.store, key)
}
