...
func (s *cardStatsWithdrawByCardService) FindMonthlyWithdrawAmountByCardNumber(ctx context.Context, req *requests.MonthYearCardNumberCard) ([]*models.MonthlyWithdrawRow, error) {
	...
	if data, found := s.cache.GetMonthlyWithdrawByNumberCache(ctx, req); found {
		logSuccess("Cache hit for monthly withdraw amount card", zap.String("card_number", observability.MaskIdentifier(req.CardNumber)), zap.Int("year", req.Year))
		return data, nil
	}
	...
	if err != nil {
		status = "error"
		return sharederrorhandler.HandleError[[]*models.MonthlyWithdrawRow](
			s.logger,
...
func (s *cardStatsWithdrawByCardService) FindYearlyWithdrawAmountByCardNumber(ctx context.Context, req *requests.MonthYearCardNumberCard) ([]*models.YearlyWithdrawRow, error) {
	...
	if data, found := s.cache.GetYearlyWithdrawByNumberCache(ctx, req); found {
		logSuccess("Cache hit for yearly withdraw amount card", zap.String("card_number", observability.MaskIdentifier(req.CardNumber)), zap.Int("year", req.Year))
		return data, nil
	}
	...
	if err != nil {
		status = "error"
		return sharederrorhandler.HandleError[[]*models.YearlyWithdrawRow](
			s.logger,
