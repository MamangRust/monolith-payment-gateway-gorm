...
func (s *cardStatsTopupByCardService) FindMonthlyTopupAmountByCardNumber(ctx context.Context, req *requests.MonthYearCardNumberCard) ([]*models.MonthlyTopupRow, error) {
	...
	if data, found := s.cache.GetMonthlyTopupByNumberCache(ctx, req); found {
		logSuccess("Cache hit for monthly topup amount card", zap.String("card_number", observability.MaskIdentifier(req.CardNumber)), zap.Int("year", req.Year))
		return data, nil
	}
	...
	if err != nil {
		status = "error"
		return sharederrorhandler.HandleError[[]*models.MonthlyTopupRow](
			s.logger,
...
func (s *cardStatsTopupByCardService) FindYearlyTopupAmountByCardNumber(ctx context.Context, req *requests.MonthYearCardNumberCard) ([]*models.YearlyTopupRow, error) {
	...
	if data, found := s.cache.GetYearlyTopupByNumberCache(ctx, req); found {
		logSuccess("Cache hit for yearly topup amount card", zap.String("card_number", observability.MaskIdentifier(req.CardNumber)), zap.Int("year", req.Year))
		return data, nil
	}
	...
	if err != nil {
		status = "error"
		return sharederrorhandler.HandleError[[]*models.YearlyTopupRow](
			s.logger,
