...
-- Durable event delivery. Producers must insert into this table in the same
-- PostgreSQL transaction as the business mutation they are announcing.
...
-- Durable consumer-side deduplication. A message is marked only after the
-- handler's side effect succeeds; malformed/non-retryable messages may be
-- recorded separately by the consumer if desired.
