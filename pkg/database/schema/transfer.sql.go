...
const createTransferAtomic = `-- name: CreateTransferAtomic :one
WITH active_recipient AS (
    ...
    JOIN saldos AS receiver_saldo ON receiver_saldo.card_number = $2
    WHERE sender_saldo.card_number = $1
      AND sender_saldo.deleted_at IS NULL
      AND receiver_saldo.deleted_at IS NULL
      AND sender_saldo.total_balance >= $3
    ...
    SELECT $1, $2, $3, current_timestamp, $4, 'success', current_timestamp, current_timestamp
...
type CreateTransferAtomicParams struct {
	TransferFrom   string `json:"transfer_from"`
	TransferTo     string `json:"transfer_to"`
	TransferAmount int32  `json:"transfer_amount"`
...
type CreateTransferAtomicRow struct {
	TransferID     int32            `json:"transfer_id"`
	TransferNo     pgtype.UUID      `json:"transfer_no"`
	TransferFrom   string           `json:"transfer_from"`
	TransferTo     string           `json:"transfer_to"`
...
func (q *Queries) CreateTransferAtomic(ctx context.Context, arg CreateTransferAtomicParams) (*CreateTransferAtomicRow, error) {
	row := q.db.QueryRow(ctx, createTransferAtomic,
		arg.TransferFrom,
		arg.TransferTo,
		arg.TransferAmount,
		arg.IdempotencyKey,
	)
	var i CreateTransferAtomicRow
	err := row.Scan(
		&i.TransferID,
		&i.TransferNo,
		&i.TransferFrom,
...
type GetTransferByIdempotencyKeyRow struct {
	TransferID     int32            `json:"transfer_id"`
	TransferNo     pgtype.UUID      `json:"transfer_no"`
	TransferFrom   string           `json:"transfer_from"`
	TransferTo     string           `json:"transfer_to"`
...
func (q *Queries) GetTransferByIdempotencyKey(ctx context.Context, idempotencyKey string) (*GetTransferByIdempotencyKeyRow, error) {
	row := q.db.QueryRow(ctx, getTransferByIdempotencyKey, idempotencyKey)
	var i GetTransferByIdempotencyKeyRow
	err := row.Scan(
		&i.TransferID,
		&i.TransferNo,
		&i.TransferFrom,
...
SELECT transfer_id, transfer_no, transfer_from, transfer_to, transfer_amount, transfer_time, status, created_at, updated_at, deleted_at, idempotency_key
FROM transfers
WHERE
    transfer_id = $1
    AND deleted_at IS NOT NULL
...
type RestoreTransferRow struct {
	TransferID     int32            `json:"transfer_id"`
	TransferNo     pgtype.UUID      `json:"transfer_no"`
	TransferFrom   string           `json:"transfer_from"`
	TransferTo     string           `json:"transfer_to"`
...
func (q *Queries) RestoreTransfer(ctx context.Context, transferID int32) (*RestoreTransferRow, error) {
	row := q.db.QueryRow(ctx, restoreTransfer, transferID)
	var i RestoreTransferRow
	err := row.Scan(
		&i.TransferID,
		&i.TransferNo,
		&i.TransferFrom,
...
func (q *Queries) TrashTransfer(ctx context.Context, transferID int32) (*TrashTransferRow, error) {
	row := q.db.QueryRow(ctx, trashTransfer, transferID)
	var i TrashTransferRow
	err := row.Scan(
		&i.TransferID,
		&i.TransferNo,
		&i.TransferFrom,
...
const updateTransferAtomic = `-- name: UpdateTransferAtomic :one
WITH locked_transfer AS MATERIALIZED (
    ...
    WHERE t.transfer_from = $2
      AND t.transfer_to = $3
      AND t.transfer_from <> t.transfer_to
      AND sender_saldo.deleted_at IS NULL
      AND receiver_saldo.deleted_at IS NULL
      AND EXISTS (SELECT 1 FROM locked_saldos)
      AND sender_saldo.total_balance + t.transfer_amount - $4 >= 0
      AND $4 > 0
), settled_saldos AS (
    UPDATE saldos s
    SET total_balance = s.total_balance + CASE
        WHEN s.card_number = t.transfer_from THEN t.transfer_amount - $4
        ELSE $4 - t.transfer_amount
    END,
    updated_at = current_timestamp
    FROM locked_transfer t
    WHERE s.card_number IN (t.transfer_from, t.transfer_to)
...
type UpdateTransferAtomicParams struct {
	TransferID     int32     `json:"transfer_id"`
	TransferFrom   string    `json:"transfer_from"`
	TransferTo     string    `json:"transfer_to"`
	TransferAmount int32     `json:"transfer_amount"`
...
type UpdateTransferAtomicRow struct {
	TransferID     int32            `json:"transfer_id"`
	TransferNo     pgtype.UUID      `json:"transfer_no"`
	TransferFrom   string           `json:"transfer_from"`
	TransferTo     string           `json:"transfer_to"`
...
func (q *Queries) UpdateTransferAtomic(ctx context.Context, arg UpdateTransferAtomicParams) (*UpdateTransferAtomicRow, error) {
	row := q.db.QueryRow(ctx, updateTransferAtomic,
		arg.TransferID,
		arg.TransferFrom,
		arg.TransferTo,
		arg.TransferAmount,
...
const updateTransferSettlementDelta = `-- name: UpdateTransferSettlementDelta :one
...
type UpdateTransferSettlementDeltaParams struct {
	CardNumber     string `json:"card_number"`
	CardNumber_2   string `json:"card_number_2"`
	TotalBalance   int32  `json:"total_balance"`
	TotalBalance_2 int32  `json:"total_balance_2"`
}
...
func (q *Queries) UpdateTransferSettlementDelta(ctx context.Context, arg UpdateTransferSettlementDeltaParams) (int32, error) {
	row := q.db.QueryRow(ctx, updateTransferSettlementDelta,
		arg.CardNumber,
		arg.CardNumber_2,
		arg.TotalBalance,
		arg.TotalBalance_2,
