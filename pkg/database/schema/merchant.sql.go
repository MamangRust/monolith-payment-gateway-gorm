...
SELECT merchant_id, merchant_no, name, api_key, user_id, status, created_at, updated_at, deleted_at, event_version
FROM merchants
WHERE
    merchant_id = $1
    AND deleted_at IS NOT NULL
...
func (q *Queries) GetYearlyPaymentMethodByMerchants(ctx context.Context, arg GetYearlyPaymentMethodByMerchantsParams) ([]*GetYearlyPaymentMethodByMerchantsRow, error) {
	rows, err := q.db.Query(ctx, getYearlyPaymentMethodByMerchants, arg.Column1, arg.MerchantID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
type RestoreMerchantRow struct {
	MerchantID int32            `json:"merchant_id"`
	MerchantNo pgtype.UUID      `json:"merchant_no"`
	Name       string           `json:"name"`
	ApiKey     string           `json:"api_key"`
...
func (q *Queries) RestoreMerchant(ctx context.Context, merchantID int32) (*RestoreMerchantRow, error) {
	row := q.db.QueryRow(ctx, restoreMerchant, merchantID)
	var i RestoreMerchantRow
	err := row.Scan(
		&i.MerchantID,
		&i.MerchantNo,
		&i.Name,
...
func (q *Queries) TrashMerchant(ctx context.Context, merchantID int32) (*TrashMerchantRow, error) {
	row := q.db.QueryRow(ctx, trashMerchant, merchantID)
	var i TrashMerchantRow
	err := row.Scan(
		&i.MerchantID,
		&i.MerchantNo,
		&i.Name,
