...
const createWithdrawAtomic = `-- name: CreateWithdrawAtomic :one
WITH active_recipient AS (
    ...
    FROM withdraws w
    WHERE w.card_number = $1
      AND w.status = 'success'
      AND w.deleted_at IS NULL
      AND w.withdraw_time >= date_trunc('day', $2::timestamp)
      AND w.withdraw_time < date_trunc('day', $2::timestamp) + interval '1 day'
), eligible AS (
    SELECT 1
    FROM saldos AS saldo
    WHERE saldo.card_number = $1
      AND saldo.deleted_at IS NULL
      AND saldo.total_balance >= $3
      ...
      AND COALESCE((
          ...
          WHERE daily_w.card_number = $1
            AND daily_w.status = 'success'
            AND daily_w.deleted_at IS NULL
            AND daily_w.withdraw_time >= date_trunc('day', $2::timestamp)
            AND daily_w.withdraw_time < date_trunc('day', $2::timestamp) + interval '1 day'
      ), 0)::BIGINT + $3 <= $4
), debited_saldo AS (
    UPDATE saldos s
    SET total_balance = s.total_balance - $3,
        withdraw_amount = $3,
        withdraw_time = $2,
    ...
    SELECT $1, $3, $2, $5, 'success', current_timestamp, current_timestamp
...
type CreateWithdrawAtomicParams struct {
	CardNumber     string    `json:"card_number"`
	WithdrawTime   time.Time `json:"withdraw_time"`
	WithdrawAmount int32     `json:"withdraw_amount"`
...
type CreateWithdrawAtomicRow struct {
	WithdrawID     int32            `json:"withdraw_id"`
	WithdrawNo     pgtype.UUID      `json:"withdraw_no"`
	CardNumber     string           `json:"card_number"`
	WithdrawAmount int32            `json:"withdraw_amount"`
...
func (q *Queries) CreateWithdrawAtomic(ctx context.Context, arg CreateWithdrawAtomicParams) (*CreateWithdrawAtomicRow, error) {
	row := q.db.QueryRow(ctx, createWithdrawAtomic,
		arg.CardNumber,
		arg.WithdrawTime,
		arg.WithdrawAmount,
		arg.DailyLimit,
...
SELECT withdraw_id, withdraw_no, card_number, withdraw_amount, withdraw_time, status, created_at, updated_at, deleted_at, idempotency_key
FROM withdraws
WHERE
    withdraw_id = $1
    AND deleted_at IS NOT NULL
...
const getWithdrawByIdempotencyKey = `-- name: GetWithdrawByIdempotencyKey :one
SELECT
    withdraw_id,
    withdraw_no,
    card_number,
...
type GetWithdrawByIdempotencyKeyRow struct {
	WithdrawID     int32            `json:"withdraw_id"`
	WithdrawNo     pgtype.UUID      `json:"withdraw_no"`
	CardNumber     string           `json:"card_number"`
	WithdrawAmount int32            `json:"withdraw_amount"`
...
func (q *Queries) GetWithdrawByIdempotencyKey(ctx context.Context, idempotencyKey string) (*GetWithdrawByIdempotencyKeyRow, error) {
	row := q.db.QueryRow(ctx, getWithdrawByIdempotencyKey, idempotencyKey)
	var i GetWithdrawByIdempotencyKeyRow
	err := row.Scan(
		&i.WithdrawID,
		&i.WithdrawNo,
		&i.CardNumber,
...
type RestoreWithdrawRow struct {
	WithdrawID     int32            `json:"withdraw_id"`
	WithdrawNo     pgtype.UUID      `json:"withdraw_no"`
	CardNumber     string           `json:"card_number"`
	WithdrawAmount int32            `json:"withdraw_amount"`
...
func (q *Queries) RestoreWithdraw(ctx context.Context, withdrawID int32) (*RestoreWithdrawRow, error) {
	row := q.db.QueryRow(ctx, restoreWithdraw, withdrawID)
	var i RestoreWithdrawRow
	err := row.Scan(
		&i.WithdrawID,
		&i.WithdrawNo,
		&i.CardNumber,
...
func (q *Queries) TrashWithdraw(ctx context.Context, withdrawID int32) (*TrashWithdrawRow, error) {
	row := q.db.QueryRow(ctx, trashWithdraw, withdrawID)
	var i TrashWithdrawRow
	err := row.Scan(
		&i.WithdrawID,
		&i.WithdrawNo,
		&i.CardNumber,
...
const updateWithdrawAtomic = `-- name: UpdateWithdrawAtomic :one
WITH locked_withdraw AS MATERIALIZED (
    SELECT w.withdraw_id, w.withdraw_amount
    FROM withdraws w
    WHERE w.withdraw_id = $1
    ...
    SET total_balance = s.total_balance + (
            (
                ...
                WHERE w0.withdraw_id = $1
            ) - $2
        ),
        updated_at = current_timestamp
    WHERE s.card_number = $3
      AND s.deleted_at IS NULL
...
type UpdateWithdrawAtomicParams struct {
	WithdrawID     int32     `json:"withdraw_id"`
	WithdrawAmount int32     `json:"withdraw_amount"`
	CardNumber     string    `json:"card_number"`
	WithdrawTime   time.Time `json:"withdraw_time"`
...
type UpdateWithdrawAtomicRow struct {
	WithdrawID     int32            `json:"withdraw_id"`
	WithdrawNo     pgtype.UUID      `json:"withdraw_no"`
	CardNumber     string           `json:"card_number"`
	WithdrawAmount int32            `json:"withdraw_amount"`
...
func (q *Queries) UpdateWithdrawAtomic(ctx context.Context, arg UpdateWithdrawAtomicParams) (*UpdateWithdrawAtomicRow, error) {
	row := q.db.QueryRow(ctx, updateWithdrawAtomic,
		arg.WithdrawID,
		arg.WithdrawAmount,
		arg.CardNumber,
		arg.WithdrawTime,
	)
	var i UpdateWithdrawAtomicRow
	err := row.Scan(
		&i.WithdrawID,
		&i.WithdrawNo,
		&i.CardNumber,
