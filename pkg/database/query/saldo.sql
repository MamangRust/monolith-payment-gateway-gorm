...
-- CreateSaldoIfNotExists: Creates the default saldo from an asynchronous card event without overwriting an existing balance
...
-- UpdateSaldoBalanceDelta: Applies an atomic delta to a card balance.
-- Purpose: Race-free balance adjustment (increment/decrement) for update flows.
-- Parameters:
--   $1: card_number - The card number to update
--   $2: balance_delta - Signed amount to add (negative debits)
-- Business Logic:
--   - Updates only active records.
--   - Rejects the update when the resulting balance would be negative
--     (returns no row; balance unchanged).
-- name: UpdateSaldoBalanceDelta :one
UPDATE saldos
SET
